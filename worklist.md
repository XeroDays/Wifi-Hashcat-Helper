# Password Wordlists Download Guide

## Wordlist 1: HK HLM Founds

**Commands:**
```bash
wget -c https://weakpass.com/download/1256/hk_hlm_founds.txt.gz
7z x hk_hlm_founds.txt.gz
```

**Details:**
- **Size:** 253MB → 389MB
- **Crack Rate:** 13.58%
- **Unique:** 12.63%
- **Popular:** 90%

---

## Wordlist 2: Triple H

**Commands:**
```bash
wget -c https://weakpass.com/download/2093/triple-h.txt.7z
7z x triple-h.txt.7z
```

**Details:**
- **Size:** 5.29GB → 21.82GB
- **Crack Rate:** 36.60%
- **Unique:** 99.25%
- **Popular:** 100%

---

## Wordlist 3: Weakpass 4

**Commands:**
```bash
wget -c https://weakpass.com/download/2012/weakpass_4.txt.7z
7z x weakpass_4.txt.7z
```

**Details:**
- **Size:** 5.02GB → 22.46GB
- **Crack Rate:** 35.01%
- **Unique:** 98.59%
- **Popular:** 99.56%

---

## Wordlist 4: hashpwn

**Commands:**
```bash
wget -c https://weakpass.com/download/2080/hashpwn.txt.7z
7z x hashpwn.txt.7z
```

**Details:**
- **Size:** 3.29GB → 14.11GB
- **Crack Rate:** 33.26%
- **Unique:** 98.16%
- **Popular:** 98.96%


---

## Wordlist 4: dictionary_private

**Commands:**
```bash
wget -c https://weakpass.com/download/1930/dictionary_private.dic.7z
7z x dictionary_private.dic.7z
```

**Details:**
- **Size:** 1.26GB → 2.66GB
- **Crack Rate:** 24.86%
- **Unique:**  22.44%
- **Popular:** 95.48%



---

## Wordlist 5: Crack Station

**Commands:**
```bash
wget -c https://crackstation.net/files/crackstation.txt.gz
7z x crackstation.txt.gz
```

**Details:**
- **Size:** 4.2GB → 15GB
- **Crack Rate:** ??%
- **Unique:**  ??%
- **Popular:** ??%



---

## Wordlist 6: all-h

**Commands:**
```bash
wget -c https://weakpass.com/download/2085/all-h.txt.7z
7z x all-h.txt.7z
```

**Details:**
- **Rank:** S
- **Size:** 6.70GB → 28.51GB
- **Crack Rate:** 38.50%
- **Unique:**  99.44%
- **Popular:** 100%





